...
nparm = 2*(numkgrid) + 3; % total number of parameters
% sunk/fixed cost for 8 capital grid  = 16
% initial condition 3

% draw the random uniform noises for each iteration
um = log(rand(nrep,2)); % 1 for IC and 1 for import cost
...
mgammaFS = [0*ones(1,numkgrid * 2)]; % mean 8 params
vgammasFS = [1000*ones(1,numkgrid * 2)]; % variance 8params
...
istart = [3.1699*ones(1,8) 85.3917*ones(1,8)];
